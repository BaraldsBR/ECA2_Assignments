...
data Axi4Stream a n = Axi4Stream {
  ...
  tLast :: Bool,
  tKeep :: n
} deriving (Generic, NFDataX, Read, ShowX)


instance (Show a, Show n) => Show (Axi4Stream a n) where
  -- instance Show is closely coupled with Axi readers and generator files.
  show axi = "(Axi4Stream (" L.++ (show $ tData axi) L.++ ") " L.++ (show $ tLast axi) L.++ " " L.++ (show $ tKeep axi) L.++ ")"
